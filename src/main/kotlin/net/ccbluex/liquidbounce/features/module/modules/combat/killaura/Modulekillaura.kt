...
object ModuleKillAura : ClientModule("KillAura", Category.COMBAT) {
    ...
    data class CombatAIContext(
        ...
        val playerMovementSpeed: Float,
        val playerCombatExperience: Float // 0.0-1.0 based on combat history
    )
    ...
    data class BehaviorProfile(
        ...
        val variability: Float = 0.5f
    ) {
        companion object {
            val DEFAULT = BehaviorProfile(0.5f, MovementStyle.NORMAL, 3.5f, 0.5f)
        }
    }
    ...
    class MovementHistoryTracker {
        ...
        private val maxHistorySize = 20 // Increased from 15 for better analysis
        ...
        fun updateEntityHistory(entity: LivingEntity) {
            ...
            val snapshot = MovementSnapshot(
                ...
                rotation = if (entity is PlayerEntity) entity.rotation else null
            )
        ...
        fun clearOldEntries() {
            ...
            entityHistory.entries.removeIf { (_, snapshots) ->
                snapshots.isEmpty() || currentTime - snapshots.last().timestamp > 10000 // 10 seconds
            }
        }
    }
    ...
    object KillAuraLearningSystem {
        ...
        suspend fun analyzePlayerBehavior(player: PlayerEntity): BehaviorProfile {
            ...
            return BehaviorProfile(
                ...
                preferredRange = avgDistance.toFloat().coerceIn(2.5f, 5.0f),
        ...
        private fun calculateAccelerations(history: List<MovementSnapshot>): List<Double> {
            ...
            return history.zipWithNext { a, b ->
                ...
                if (dt > 0) {
                    val dv = b.velocity.subtract(a.velocity)
                    dv.length() / dt // acceleration magnitude
                } else {
                    0.0
                }
            }
        ...
        private fun determineDetailedMovementStyle(
            ...
            return when {
                ...
                aggressiveScore > 0.3 + randomFactor -> MovementStyle.AGGRESSIVE
                defensiveScore > 0.3 + randomFactor -> MovementStyle.DEFENSIVE
        ...
        private fun calculateMovementVariability(
            ...
            val timeVariance = if (timeDeltas.isNotEmpty()) {
                val avg = timeDeltas.average()
                timeDeltas.map { (it - avg).pow(2) }.average() / avg.pow(2)
            } else {
                0.0
            }
            ...
            val accelVariance = if (accelerations.isNotEmpty()) {
                ...
                accelerations.map { (it - avg).pow(2) }.average() / avg.pow(2)
            } else {
                0.0
            }
            ...
            val jerkVariance = if (jerks.isNotEmpty()) {
                ...
                jerks.map { (it - avg).pow(2) }.average() / avg.pow(2)
            } else {
                0.0
            }
        ...
        suspend fun calculateAdaptiveThreatScore(enemy: LivingEntity, baseScore: Float): Float {
            ...
            if (att <= 0) return baseScore
            val rate = suc.toFloat() / att.toFloat()
            return baseScore * (1.0f + (rate - 0.5f) * 0.3f)
        }
    }
    ...
    object KillAuraAIPredictor {
        ...
        suspend fun getOptimalTarget(context: CombatAIContext): LivingEntity? {
            ...
            // Add some randomness to target selection based on player's combat experience
            ...
            return when {
                ...
                random < 0.9 * experienceFactor && targetScores.size >= 2 -> targetScores[1].first
                targetScores.size >= 3 -> targetScores[2].first
                else -> targetScores.firstOrNull()?.first
            }
        }
        ...
        suspend fun predictTargetPosition(target: Entity, ticksAhead: Int): Vec3d {
            ...
            val predictedVelocity = lastVelocity.add(acceleration.multiply(0.5))
    ...
    private val scanExtraRange by floatRange("ScanExtraRange", 1.0f..1.5f, 0.0f..5.0f).onChanged { r ->
    ...
    private val lastTargetSwitchTime = mutableMapOf<String, Long>()
    ...
    private val gameHandler = tickHandler {
        ...
        if (targetTracker.target != null && combatStartTime == 0L) {
            combatStartTime = System.currentTimeMillis()
        } else if (targetTracker.target == null) {
            combatStartTime = 0L
        }

        // Update movement history for all entities
        ...
        // Use AI to predict position and optimize attack
        ...
        if (learningEnabled) {
            KillAuraAIPredictor.updatePredictionAccuracy(finalTarget)
        }

        // ===== IMPROVED TARGET SELECTION =====
        val crosshairTarget = if (samePlayer && targetTracker.stickyTarget != null) {
            // If SamePlayer is enabled and we have a sticky target, always use that target as the final target.
            targetTracker.stickyTarget
        } else {
            // Check if we should switch targets based on timing
            val currentTime = System.currentTimeMillis()
            ...
            if (currentTime - lastSwitchTime < switchDelayWithRandomization) {
                ...
                if (raycastTarget is LivingEntity && raycastTarget.shouldBeAttacked() && raycastTarget != finalTarget) {
                    // Apply target switching logic with hysteresis
                    if (Random.nextFloat() < 0.7f) { // 70% chance to switch to new target
                        raycastTarget
                    } else {
                        finalTarget
                    }
        ...
        // Only perform attack if we have a valid final target
        if (crosshairTarget != null) {
            // Apply humanized rotation for attack
            val humanizedRotation = humanizeRotation(rotation)
            attackTarget(this, crosshairTarget, humanizedRotation)
        }
        // ===== END OF IMPROVED TARGET SELECTION =====
    }
    ...
    private suspend fun attackTarget(sequence: Sequence, target: Entity, rotation: Rotation) {
        ...
        if (clickScheduler.isClickTick && validateAttack(target) && !shouldMiss) {
            clickScheduler.attack(sequence, rotation) {
                ...
                // AI learns from result (called in coroutine)
    ...
    private fun createNonLinearPath(from: Rotation, to: Rotation, steps: Int): List<Rotation> {
        ...
        // Limit target with rotation speed limits
     ...
     * This prevents the 360° rotation issue by limiting delta per elapsed time
     */
     ...
     * Ensures shortest rotation path (no wrap-around spin)
     */
    private fun getSmallestAngleDifference(from: Float, to: Float): Float {
     ...
     * Returns new yaw value (current + limited delta + small random)
     */
    private fun getHumanizedShortestAngle(current: Float, target: Float): Float {
        ...
        // Add micro-randomization to avoid perfect patterns
    ...
    private fun createAIContext(): CombatAIContext {
        return CombatAIContext(
            ...
            playerMovementSpeed = player.velocity.length().toFloat(),
            playerCombatExperience = KillAuraLearningSystem.getCombatExperience()
        )
    }
    ...
    private fun updateTarget() {
        ...
        val target = if (currentTarget != null && sortedTargets.any { it.first == currentTarget }) {
            if (Random.nextDouble() < 0.85) {
                ...
                sortedTargets.firstOrNull { it.first != currentTarget && processTarget(it.first, maximumRange, situation) }?.first
                    ?: sortedTargets.firstOrNull { processTarget(it.first, maximumRange, situation) }?.first
            }
        } else {
            sortedTargets.firstOrNull { processTarget(it.first, maximumRange, situation) }?.first
        }
    ...
    private fun calculateVariableRange(baseRange: Float): Float {
        ...
        val combatFactor = (1.0 - min(combatDuration / 30f, 0.3)).toFloat()
    ...
    private fun processTarget(
        ...
        val correctedRotation = Rotation(
            getHumanizedShortestAngle(currentRotation.yaw, rotation.yaw),
            rotation.pitch.coerceIn(-90f, 90f)
        )
