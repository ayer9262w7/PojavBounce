...
class HumanHybridAngleSmooth(parent: ChoiceConfigurable<*>) : AngleSmooth("HumanHybrid", parent) {
    ...
    override fun process(
        ...
        // FIX 360°: Tính bước nhỏ nhất cần xoay, không cho vượt quá maxStep, và wrap kết quả cuối cùng
        val yawDiffToTarget = RotationUtil.angleDifference(targetRotation.yaw, currentRotation.yaw)
        val pitchDiffToTarget = RotationUtil.angleDifference(targetRotation.pitch, currentRotation.pitch)

        val newYaw = MathHelper.wrapDegrees(
            currentRotation.yaw + yawDiffToTarget.coerceIn(-abs(yawStep), abs(yawStep))
        )
        val newPitch = (currentRotation.pitch + pitchDiffToTarget.coerceIn(-abs(pitchStep), abs(pitchStep)))
            .coerceIn(-90f, 90f)
    ...
    override fun calculateTicks(currentRotation: Rotation, targetRotation: Rotation): Int {
        ...
        // Lưu ý: dùng độ chênh nhỏ nhất
        val ticksH = floor(abs(RotationUtil.angleDifference(targetRotation.yaw, currentRotation.yaw)) / abs(yawStep)).let { if (it.isNaN()) 0.0f else it.toFloat() }
        val ticksV = floor(abs(RotationUtil.angleDifference(targetRotation.pitch, currentRotation.pitch)) / abs(pitchStep)).let { if (it.isNaN()) 0.0f else it.toFloat() }
